...
<div class="container">
	...
	<div class="form-group">
		<label class="col-sm-2" for="description">ที่อยู่ </label> 
		<div class="col-sm-10">
			${dealer.address}
		</div>
	</div>
	<div class="form-group">
		<label class="col-sm-2" for="phone">เบอร์ติดต่อ </label>
		<div class="col-sm-10"> 
			${dealer.phone}
		</div>
	</div>
	...
	<table class="table table-hover table-bordered">
		...
		<c:forEach items="${stocks}" var="stock">
			<tr>	
				...
				<td>
					<sf:form action="${pageContext.request.contextPath}/stock/dealer/delete?dealerId=${dealer.id}&stockId=${stock.id }" method="delete" cssClass="delete" id="${dealer.id}_${stock.id }">
                        <button type="button" class="btn btn-danger" onclick="javascript:onDelete('${dealer.id}', '${stock.id}');">ลบ</button>
                    </sf:form>
				</td>
			</tr>
		</c:forEach>
...
<%-- 	</c:if> --%>
	<button type="button" class="btn btn-danger" onclick="javascript:window.location.href='${pageContext.request.contextPath}/stock/dealer-for-update?dealerId=${dealer.id}'">ตัด Stock</button>
...
function onDelete(dealerId, stockId){
    $('#confirm').modal({ backdrop: 'static', keyboard: false })
    .one('click', '#delete', function (e) {
    	$('#' + dealerId + '_' + stockId).submit();
    });
}
</script>
</body>
